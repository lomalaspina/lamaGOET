...
CP2K_VALIDATE_LAMAGOET_MODE() {
    ...
    if [[ "$(_lower "${XWR:-false}")" == "true" && "$(_lower "${XCW_MODE:-molecular}")" != "periodic" ]]; then
        _cp2k_error "CP2K XWR requires XCW_MODE=periodic; the molecular XCW path cannot consume a periodic wavefunction"
        return 1
    fi
    return 0
}
...
CP2K_FINAL_RESIDUALS() {
    ...
    for artifact in "${JOBNAME}.periodic.trexio" "${JOBNAME}.periodic.trexio.json" "${JOBNAME}.periodic-wavefunction-export.log"; do
        if [[ -f "$artifact" ]]; then
            cp "$artifact" "final.CP2K.residuals.${JOBNAME}/$artifact"
        fi
    done
...
CP2K_RUN_HAR() {
    ...
    # XWR is HAR followed by XCW. CP2K does not expose the overlap, Fock and
    # orbital matrices required by periodic XCW, so RUN_XWR creates a fresh
    # mutually consistent Crystal23 reference at this final HAR geometry.
    if [[ "$(_lower "${XWR:-false}")" == "true" ]]; then
        _cp2k_log "Starting fixed-geometry periodic XCW at the final CP2K-HAR geometry"
        RUN_XWR || return 1
    fi
    duration=$SECONDS
	...
	case "$SCFCALCPROG" in
		...
		Crystal14)
			...
			if [[ -n "${CRYSTAL_LDREMO:-}" ]]; then
				# CRYSTAL defines LDREMO=n as removal of overlap directions below
				# n x 10^-5.  Apply the identical rank threshold rather than
				# inventing orbitals in the discarded null space.
				crystal_export_options+=(
...
#	        READ_CRYSTAL_WFN
	...
	if [[ "$SCFCALCPROG" != "elmodb" && "$SCFCALCPROG" != "optgaussian" && "$SCFCALCPROG" != "optorca" && "$POWDER_HAR" != "true" ]]; then
		...
		DEFINE_JOB_NAME
               if [[ "$SCFCALCPROG" == "Crystal14" ]]; then
			# Native GRED is self-describing and Tonto imports its exact
			# atom-resolved Gaussian basis.  Only the legacy XML interface
			# needs a separately selected Tonto reconstruction basis.
			if ! CRYSTAL_GRED_IMPORT_SUPPORTED; then
				NOT_TONTO_BASIS_SET
			fi
        	        CHARGE_MULT
