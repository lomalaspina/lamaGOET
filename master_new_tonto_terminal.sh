...
echo "Which software for SCF steps, Gaussian or Orca [Gaussian]"
read SCFCALCPROG
...
echo "Basis set that you wish to use (Gaussian or Orca format) [STO-3G]"
echo ""
read BASISSET
if [[ -z "$BASISSET" ]]; then
	BASISSET=$"STO-3G"
fi
echo "Method that you wish to use (Gaussian or Orca format) [HF]"
echo ""
read METHOD
if [[ -z "$METHOD" ]]; then
	METHOD=$"HF"
