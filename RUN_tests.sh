...
#	echo "Gamess cycle number $I ended"
	...
	if ! grep -q 'OVERLAP INTEGRALS WRITTEN ON FILE sao' "$JOBNAME.gamess.out"; then
		echo "ERROR: Calculation of overlap integrals with gamessus finished with error, please check the $I.th gamess.out file for more details" >> $JOBNAME.lst
		unset MAIN_DIALOG
		exit 0
	else 
	...
	#	fi
		...
		if [ "$REFANHARM" == "true" ]; then
			if [[ "$THIRDORD" == "true" && "$FOURTHORD" == "true" ]]; then
				...
				echo "ERROR: Please select at least one of the anharmonic terms to refine" >> $JOBNAME.lst
				unset MAIN_DIALOG
				exit 0
			fi
		fi
...
#	echo "Tonto cycle number $J ended"
	if ! grep -q 'Wall-clock time taken' "stdout"; then
		echo "ERROR: problems in fit cycle, please check the $J.th stdout file for more details" >> $JOBNAME.lst
		unset MAIN_DIALOG
		exit 0
	fi
	if [ $J = 1 ]; then 
...
#	echo "Generation fcheck file for Gaussian cycle number $I"
	formchk $JOBNAME.chk $JOBNAME.fchk  >/dev/null
     	mkdir $I.$SCFCALCPROG.cycle.$JOBNAME
...
#		else
			...
			if [ "$ONF" = "true" ]; then
				...
				echo "ERROR: Please select the format of the hkl file for header (F or F^2)" >> $JOBNAME.lst
				unset MAIN_DIALOG
				exit 0
			fi
	...
	if [[ -z "$(grep "reflection_data= {" $HKL)" ]]; then
		echo "You are missing the tonto header in the hkl file." >> $JOBNAME.lst
	fi
...
#		echo "Done reading cif with Tonto"
	...
	#######cp $JOBNAME'_cartesian.cif2' $JOBNAME.$J.cartesian.cif2
		if [ "$SCFCALCPROG" = "Gaussian" ]; then 
			...
			echo "%rwf=./$JOBNAME.rwf" | tee -a $JOBNAME.com  $JOBNAME.lst >/dev/null
			echo "%int=./$JOBNAME.int" | tee -a $JOBNAME.com  $JOBNAME.lst >/dev/null
			echo "%mem=$MEM" | tee -a $JOBNAME.com  $JOBNAME.lst >/dev/null
			echo "%nprocshared=$NUMPROC" | tee -a $JOBNAME.com $JOBNAME.lst >/dev/null
			#this is the first SCF so no charges in!
			if [ "$METHOD" = "rks" ]; then
				echo "# blyp/$BASISSET nosymm output=wfn 6D 10F $INT" | tee -a $JOBNAME.com $JOBNAME.lst >/dev/null   
			else
				if [ "$METHOD" = "uks" ]; then
					echo "# ublyp/$BASISSET nosymm output=wfn 6D 10F $INT" | tee -a $JOBNAME.com $JOBNAME.lst >/dev/null
				else
					echo "# $METHOD/$BASISSET nosymm output=wfn 6D 10F $INT" | tee -a $JOBNAME.com $JOBNAME.lst >/dev/null
			        fi
			fi			
			echo ""  | tee -a $JOBNAME.com $JOBNAME.lst >/dev/null
			echo "$JOBNAME" | tee -a $JOBNAME.com $JOBNAME.lst >/dev/null
			echo "" | tee -a  $JOBNAME.com $JOBNAME.lst >/dev/null
			echo "$CHARGE $MULTIPLICITY" | tee -a  $JOBNAME.com $JOBNAME.lst >/dev/null
			### commented but working on new tonto, will use dylan's old write_xyz_file keyword because that one keeps the atom labels, florian's new one does not.
			...
			awk 'NR>2' $JOBNAME.xyz | tee -a  $JOBNAME.com $JOBNAME.lst >/dev/null
			### old working
			...
			echo "" | tee -a $JOBNAME.com  $JOBNAME.lst >/dev/null
			if [ "$GAUSGEN" = "true" ]; then
		        	cat basis_gen.txt | tee -a $JOBNAME.com  $JOBNAME.lst >/dev/null
				echo "" | tee -a $JOBNAME.com  $JOBNAME.lst >/dev/null
			fi
		 	#if [ "$SCCHARGES" = "true" ]; then 
			...
			#	echo "" | tee -a $JOBNAME.com  $JOBNAME.lst
			#fi
			echo "./$JOBNAME.wfn" | tee -a $JOBNAME.com  $JOBNAME.lst >/dev/null
			echo "" | tee -a $JOBNAME.com  $JOBNAME.lst >/dev/null
...
#			echo "Gaussian cycle number $I, final energy is: $ENERGIA, RMSD is: $RMSD "
		...
		###################### Begin first tonto fit ########################
			...
			echo "%output" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
	 		echo "   PrintLevel=Normal" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
	 		echo "   Print[ P_Basis       ] 2" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
	 		echo "   Print[ P_GuessOrb    ] 1" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
	 		echo "   Print[ P_MOs         ] 1" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
	 		echo "   Print[ P_Density     ] 1" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
	 		echo "   Print[ P_SpinDensity ] 1" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
			echo "end" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
			echo "" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
			echo "* xyz $CHARGE $MULTIPLICITY" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
			awk 'NR>2' $JOBNAME.xyz | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
			echo "*" | tee -a $JOBNAME.inp $JOBNAME.lst >/dev/null
			I=$"1"
#			echo "Running Orca, cycle number $I" 
	 		$SCFCALC_BIN $JOBNAME.inp > $JOBNAME.log  
