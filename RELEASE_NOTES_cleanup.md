...
- CP2K and Crystal23 always retain their imported theoretical density and
  expose only the finite-cluster versus periodic stockholder selector. Tonto
  alone offers its standard `oc-hirshfeld` density or the experimental
  `oc-observed` prototype; shrinkage, minimum thermal-factor, and zero-phase
  controls appear only when the observed Tonto model is selected.
