...
#
#    gnuplot -persist 'my_job.Delta_F_pred_z_vs_F_pred.gnuplot'
#
...
set yrange [-0.200000E+01:0.100000E+02]

# Plot
plot 'my_job.Delta_F_pred_z_vs_F_pred' using 1:2 \
        with points pt 7 ps 0.3 lc rgb 'dark-violet', \
     'my_job.Delta_F_pred_z_vs_F_pred.labels' using 1:2:($5-$1):($6-$2) \
        with vectors nohead lw 1 lc rgb 'gray50', \
     'my_job.Delta_F_pred_z_vs_F_pred.labels' using 3:4:7 \
        with labels center font ',8' tc rgb 'red'

# PDF output
# Uncomment, or do it from the Qt window
# set terminal pdfcairo size 5,5
# set output 'my_job.Delta_F_pred_z_vs_F_pred.gnuplot.pdf'
# replot
